...
class Trainer:
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            format(self.epoch_num+1, 0, self.val_losses[-1], 
            0, sum(self.val_aucs[-1])/self.n_classes))

            losses = [self.train_losses[-1], self.val_losses[-1]]
            self.loss_df.loc['Epoch {}'.format(self.epoch_num+1)] = np.array(losses)

            train_auc = self.train_aucs[-1]
            train_auc = train_auc.tolist()
            train_auc.append(sum(train_auc)/len(train_auc))
            self.train_auc_df.loc['Epoch {}'.format(self.epoch_num+1)] = train_auc

            val_auc = self.val_aucs[-1]
            val_auc = val_auc.tolist()
            val_auc.append(sum(val_auc)/len(val_auc))
            self.val_auc_df.loc['Epoch {}'.format(self.epoch_num+1)] = val_auc
            best_val_auc = max(best_val_auc, sum(val_auc)/len(val_auc))  
            print(f'best_cal_auc = {best_val_auc}')              
